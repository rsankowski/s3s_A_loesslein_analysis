...
colnames(counts) <- c("gene_id", "1_BCG", "1_C", "2_BCG", "2_C","3_BCG", "3_C")
...
counts <- counts[,-1]
coldata <- data.frame(condition=c(rep(c("BCG","CTRL"),3)))
...
## re-order
dds$condition <- factor(dds$condition, levels = c("CTRL","BCG"))
